...
async function testRaindropAPI() {
  ...
  if (apiKey === 'your-api-key-here') {
    console.log('❌ Пожалуйста, укажите ваш API ключ Raindrop.io в файле test-raindrop.ts');
    return;
  }
  ...
  try {
    ...
    rootCollections.forEach(col => {
      console.log(`   - ${col.title} (ID: ${col._id}, количество: ${col.count || 0})`);
    });
    ...
    childCollections.forEach(col => {
      console.log(`   - ${col.title} (ID: ${col._id}, родитель: ${col.parent?.$id || 'нет'})`);
    });
